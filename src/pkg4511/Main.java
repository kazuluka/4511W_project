...
//  Node[][] possibleNodes = new Node[400][400]; //these are possible nodes for camera placement
    ...
    public void paintComponent(Graphics g) {
      ...
      //**This will mark and add to possibleNodes[] nodes that can become cameras.**//
      //**Only floor nodes that are adjacent to walls are possible.**//
        if (run) {                                    
            for (int y = 0; y < img_h - 1; y++) {          
                for (int x = 0; x < img_w - 1; x++) {
                  ...
                  //holding variables for color values of touching nodes
                    rgb2 = 0; //right of x,y
                    rgb3 = 0; //above x,y
                    rgb4 = 0; //left of x,y
                    rgb5 = 0; //below x,y
                  ...
                  //**The following if statements set the orientation of the possible camera**//
                  //**node depending on which side the wall is  |  ffc0c0c0 is silver grey**//
                    if (Integer.toHexString(rgb).equals("ffc0c0c0") && rgb4 != rgb && x != 0) {
                        g2d.drawLine(x, y, x, y);                                              
                        direction = "right";
                        addNode = true;
                    }
